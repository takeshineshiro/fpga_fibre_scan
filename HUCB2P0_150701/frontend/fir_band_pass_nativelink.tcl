...
catch {set coef_files [glob -nocomplain fir_band_pass_coef*.txt]} error_msg
puts "$error_msg"
  
# The top-level in HDL type "VHDL"
set ipfs_ext vho
if {[file exists fir_band_pass.vho]} {
	set hdl_ext vhd
} else {
	puts "Warning: Could not find fir_band_pass.$ipfs_ext!"
}
...
set_global_assignment -name EDA_TEST_BENCH_MODULE_NAME work.tb_fir_band_pass -section_id tb
set_global_assignment -name EDA_TEST_BENCH_GATE_LEVEL_NETLIST_LIBRARY work -section_id tb

# IPFS file
set_global_assignment -name EDA_IPFS_FILE fir_band_pass.$ipfs_ext -section_id eda_simulation -library work
...
set_global_assignment -name EDA_TEST_BENCH_FILE fir_band_pass_input.txt -section_id tb -library work

set_global_assignment -name EDA_TEST_BENCH_FILE tb_fir_band_pass.vhd -section_id tb -library work
